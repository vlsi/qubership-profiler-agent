...
**Schema evolution.** Adding a column to `CallV2` is not a transparently backward-readable change: `xitongsys/parquet-go` aligns the footer schema by column index, so reading an older, narrower file with the newer struct panics. A schema change therefore routes to a new `v2` path segment (§7) and needs a version-aware reader that reads each file with the struct its path version names, before it is safe against the 30-day retention window where old and new files coexist. Until that reader exists, `CallV2` is frozen once real data is written. There is no production data yet, so the Stage 1 columns (`big_params_json` and the rest) were added in place without migration.
...
| Removed `non_blocking_ms` | No wire source: `writeCall` never emits it and the Go decoder has no field for it (`Dumper.java:1059-1108`, `backend/libs/parser/pipe/calls.go`). Re-adding a column later is a schema change, not free — see Schema evolution in §5.2. |
