...
| R11 | B | Aggregate node params into a mini-tree at merge. Group high-cardinality values (SQL, binds) by a normalised signature, keep the top-N groups by time with per-group `durationMs` / `executions`, bucket the rest into `::other`, and nest binds under their SQL. | A single node can hold thousands of SQL texts; shipping them raw defeats the merge. The old UI capped at top-256 + `::other` and grouped similar SQL by stripping string literals and digits (`profiler-ui/src/profiler.mjs:3469`). **Formalise the exact contract before coding** — top-N limit, per-group duration/execution attribution, who creates `::other`, and bind nesting. The SQL aggregation lives in the Java `parsers/` code, not in `profiler.mjs` (whose `::other` at line 4128 is dataflow-specific); extract the precise schema and examples from there. |
