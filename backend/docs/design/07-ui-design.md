...
| D5 (derived) | **TypeScript, not JavaScript.** Kotlin/JS ruled out (backend is Go). | AntD and RTK Query are TypeScript-first; the MessagePack decoder and the frozen-query contract earn static types. |
| D6 (derived) | **Plain AntD 4.24 only.** Drop `@netcracker/ux-react`, `@netcracker/cse-ui-components`, `@netcracker/ux-assets`. | The archived registry is unreachable for external contributors; `profiler-plan.md` §"Stage 5" mandates the removal. |
...
`pk` serialises into a path segment as
...
### 3.2 Behaviours the UI must handle
...
1. **Hotspots** — flat profile: walk the tree, aggregate `selfMs` by method, sort descending. When
   ...
   share of time (and its SQL) reads directly.
...
5. **Setup categories** — assign a category to a method by pattern; it propagates down the whole subtree
   (a child assignment overrides), colouring it. This is what lets Hotspots split time by business
...
- **Parameters** — a node's params are a mini-tree, not flat values: many SQL texts and binds per node,
  aggregated (top-N by time + `::other`), with similar SQL grouped by a normalised signature (literals and
  digits stripped, `profiler.mjs:3469`) and binds nested under their SQL. Rendered inline in the tree and
  summarised in the Parameters tab. The aggregation is server-side (doc 08 R11).
...
- **Export** — download the tree page; xlsx and stackcollapse/flamegraph export.

Open-in-IDE is dropped for the in-cluster UI (it needs a local IDE bridge).

### 5.4 Rendering

- **Virtualisation.** Flatten the visible (expanded) nodes into a list and window it (for example
  `react-window`), so a 5,000-node tree renders a constant number of rows. Param mini-trees make row heights
  variable, so the virtualiser must support dynamic heights, or params render in an expandable panel outside
