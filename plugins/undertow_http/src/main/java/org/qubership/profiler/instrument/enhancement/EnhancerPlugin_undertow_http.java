...
public class EnhancerPlugin_undertow_http extends EnhancerPlugin {
    ...
    public boolean accept(ClassInfo info) {
        ...
        if (jarName.contains("undertow")) {
            ...
            log.info("Undertow jar version is {}.{}, run undertow_http plugins = {}",
                    majorVersion, minorVersion, majorVersion >= 2 && minorVersion <= 2);
            return majorVersion >= 2 && minorVersion <= 2;
        }

        log.info("Jar has no 'undertow' in the name, plugin undertow_http will skip");
        return false;
    }
}
